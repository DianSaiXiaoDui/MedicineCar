...
#define DC_RATIO_MIN 0.05//�???????????大电机转速（比例，范�???????????0~1�???????????
...
extern uint8_t TargetIdx;//CCD识别红线中心
extern uint16_t T_location;
extern uint8_t distance_flag;//�???????????始累积距离标�???????????
extern uint8_t MoveFlag;
extern uint16_t ADV[128]; //CCD采集数组
extern uint16_t filtered_ADV[128]; // 中�?�滤波后CCD数据
extern int16_t DxMax, DxMin; //灰度值差分最大�?��?�最小�??
extern int16_t dX[188];  //灰度值差分数�?????????
extern uint16_t MaxIdx, MinIdx; //跳变点索�?????????
